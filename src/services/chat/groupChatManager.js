...
export const ensureHostelGroupChat = async hostelId => {
  ...
  try {
    ...
    let groupChat = await Chat.findOne({
      $and: [{ participants: { $elemMatch: { hostel: hostelId } } }, { group: true }],
    });
    ...
    if (!groupChat) {
      ...
      console.log(
        `Created new group chat for hostel ${hostel.name} with ${participants.length - 1} guests`
      );
    } else {
      ...
      const newGuests = hostel.user_id_guests.filter(
        guestUserId => !existingGuestIds.includes(guestUserId.toString())
      );

      if (newGuests.length > 0) {
        const newParticipants = newGuests.map(guestUserId => ({
          user: guestUserId,
        }));
        ...
        console.log(
          `Added ${newGuests.length} new guests to existing group chat for hostel ${hostel.name}`
        );
      }
    }
...
export const addGuestToHostelGroup = async (hostelId, guestUserId) => {
  try {
    ...
    const isAlreadyInGroup = groupChat.participants.some(
      p => p.user && p.user.toString() === guestUserId.toString()
    );
...
export const initializeAllHostelGroupChats = async () => {
  try {
    ...
    console.log(
      `Group chat initialization complete: ${successCount} success, ${errorCount} errors`
    );
