...
export const initiateHostelGuestChat = async (hostelId, userId) => {
  try {
    ...
    if (!privateChat) {
      ...
      console.log(
        `Created new private chat between hostel ${hostel.name} and guest ${guest.email}`
      );
    } else {
      console.log(
        `Private chat already exists between hostel ${hostel.name} and guest ${guest.email}`
      );
    }
  ...
  try {
    ...
    let privateChat = await Chat.findOne({
      $and: [
        ...
        {
          participants: { $not: { $elemMatch: { hostel: { $exists: true } } } },
        }, // Garante que não há hostels
      ],
    });
    ...
    if (!privateChat) {
      ...
      await privateChat.save();
      console.log(
        `Created new private chat between guests ${guest1.username || guest1.email} and ${guest2.username || guest2.email}`
      );
    } else {
      console.log(
        `Private chat already exists between guests ${guest1.username || guest1.email} and ${guest2.username || guest2.email}`
      );
    }
...
export const getUserPrivateChats = async userId => {
  try {
    const privateChats = await Chat.find({
      $and: [{ participants: { $elemMatch: { user: userId } } }, { group: false }],
    })
...
export const getHostelPrivateChats = async hostelId => {
  try {
    const privateChats = await Chat.find({
      $and: [{ participants: { $elemMatch: { hostel: hostelId } } }, { group: false }],
    })
      .populate('participants.user', 'username email profilePicture')
      .populate('participants.hostel', 'name location profilePicture')
      .sort({ updatedAt: -1 });
