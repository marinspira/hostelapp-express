...
async function main() {
    ...
    const room = await Room.findOne({ hostel: new mongoose.Types.ObjectId(TARGET_HOSTEL_ID), 'beds.reservation_id': null }).lean();
        ...
        console.log(`✅ Reservation created (${reservationId}) for guest ${user.email} at hostel ${updatedHostel.name}`);
